...
from ._proto import Prototype
from ._objc2 import Id, OBJC, STR, SUBCLASS, BIND, SEL, FIT
...
NSEventModifierFlagCapsLock = 1 << 16  # caps
NSEventModifierFlagShift = 1 << 17  # shift
NSEventModifierFlagControl = 1 << 18  # ctrl
NSEventModifierFlagOption = 1 << 19  # alt
NSEventModifierFlagCommand = 1 << 20  # command
NSEventModifierFlagNumericPad = 1 << 21  # numeric
NSEventModifierFlagHelp = 1 << 22  # help
NSEventModifierFlagFunction = 1 << 23  # fn
NSEventModifierFlagDeviceIndependentFlagsMask = 0xffff0000


def _flag(event):
    _modifiers = OBJC(event, 'modifierFlags')
    _shift = _modifiers & NSEventModifierFlagShift
    _ctrl = (_modifiers & NSEventModifierFlagControl or
             _modifiers & NSEventModifierFlagCommand)
    _alt = _modifiers & NSEventModifierFlagOption
...
class Window(Prototype):
    ...
    def _prepare_objc_class(self):
        ...
        def do(cls, sel, notification):
            ...
            self._window_push(
                input.mouse_enter, _origin_x, _origin_y
            )
        ...
        def do(cls, sel, notification):
            _position = OBJC(self._view, 'convertPoint:fromView:',
                             OBJC(notification, 'locationInWindow'), None)
            self._window_push(
                input.mouse_exit, _position.mem_0, _position.mem_1
            )
        ...
        def do(cls, sel, notification):
            ...
            self._window_push(
                input.mouse_move,
                _origin_x - input.mouse_x,
                _origin_y - input.mouse_y
            )
        ...
        def do(cls, sel, notification):  # 鼠标用不到纵向，但是触控板会检测横向
            ...
            if _delta_x or _delta_y:  # 触控板会误触这个事件，这里排除误触
                self._window_push(
                    input.mouse_scroll_wheel, _delta_x, _delta_y
                )
        ...
        def do(cls, sel, notification):
            _position = OBJC(self._view, 'convertPoint:fromView:',
                             OBJC(notification, 'locationInWindow'), None)
            self._window_push(
                input.mouse_down, _position.mem_0, _position.mem_1
            )
        ...
        def do(cls, sel, notification):
            _position = OBJC(self._view, 'convertPoint:fromView:',
                             OBJC(notification, 'locationInWindow'), None)
            self._window_push(
                input.mouse_up, _position.mem_0, _position.mem_1
            )
        ...
        def do(cls, sel, notification):
            _position = OBJC(self._view, 'convertPoint:fromView:',
                             OBJC(notification, 'locationInWindow'), None)
            self._window_push(
                input.right_down, _position.mem_0, _position.mem_1
            )
        ...
        def do(cls, sel, notification):
            _position = OBJC(self._view, 'convertPoint:fromView:',
                             OBJC(notification, 'locationInWindow'), None)
            self._window_push(
                input.right_up, _position.mem_0, _position.mem_1
            )
        ...
        def do(cls, sel, notification):
            _position = OBJC(self._view, 'convertPoint:fromView:',
                             OBJC(notification, 'locationInWindow'), None)
            self._window_push(
                input.other_down, _position.mem_0, _position.mem_1
            )
        ...
        def do(cls, sel, notification):
            _position = OBJC(self._view, 'convertPoint:fromView:',
                             OBJC(notification, 'locationInWindow'), None)
            self._window_push(
                input.other_up, _position.mem_0, _position.mem_1
            )
        ...
        def do(cls, sel, notification):
            ...
            self._window_push(
                input.resize, int(_area.mem_0), int(_area.mem_1)
            )
    ...
    def _app_event_hook(self):
        while True:
            ...
            if (_event_type == NSEventTypeKeyDown and
                    not OBJC(_event, 'isARepeat')):
                self._window_push(
                    input.key_press, OBJC(_event, 'keyCode'), _flag(_event)
                )
            elif _event_type == NSEventTypeKeyUp:
                self._window_push(
                    input.key_release, OBJC(_event, 'keyCode'), _flag(_event)
                )
